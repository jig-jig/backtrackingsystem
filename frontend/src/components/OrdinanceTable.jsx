...
export default function OrdinanceTable(props) {
  ...
  const [selectedOrdinanceForModal, setSelectedOrdinanceForModal] =
    useState(null);
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <div className="sm:col-span-2 relative">
          ...
          <button
            type="submit"
            className="absolute right-2 top-1.5 bg-slate-200 text-xs px-2.5 py-1 rounded font-medium hover:bg-slate-300 transition"
          >
            Go
          </button>
        </div>
      ...
      {/* FEEDBACK LABELS */}
      {error && (
        <div className="text-sm text-red-600 bg-red-50 p-3 border border-red-100 rounded-lg">
          {error}
        </div>
      )}
      ...
      {isLoading ? (
        <div className="text-center py-12 text-slate-400 text-sm">
          Syncing repository contents...
        </div>
      ) : ordinances.length === 0 ? (
        <div className="text-center py-12 text-slate-400 text-sm">
          No matching ordinance data profiles discovered.
        </div>
      ) : (
            ...
            {ordinances.map((ord) => (
              <div key={ord.id} className="p-4 space-y-2">
                <div className="flex justify-between items-start">
                  ...
                  <span
                    className={`text-[11px] font-semibold px-2 py-0.5 rounded-full border ${getStatusStyle(ord.status)}`}
                  >
                    {ord.status}
                  </span>
                </div>
            ...
            <table className="w-full text-left border-collapse text-sm">
              ...
              <tbody className="divide-y divide-slate-100 text-slate-700">
                {ordinances.map((ord) => (
                  <tr
                    key={ord.id}
                    className="hover:bg-slate-50/60 transition duration-100"
                  >
                    ...
                    <td className="p-4">
                      <span
                        className={`text-xs font-semibold px-2.5 py-0.5 rounded-full border ${getStatusStyle(ord.status)}`}
                      >
                        {ord.status}
                      </span>
                    </td>
      ...
      <div className="flex justify-between items-center bg-white p-3 border border-slate-200 rounded-xl shadow-xs">
        <span className="text-xs text-slate-400 font-medium">
          Showing Page{" "}
          <b className="text-slate-700">{paginationInfo.current_page}</b> of{" "}
          {paginationInfo.total_pages} ({paginationInfo.total_items} total
          records)
        </span>
